...
#ifndef CONFIG_S21B_AGG_TASK_WORK
#define AGG_WORK 200 // valor por defecto
#else
#define AGG_WORK CONFIG_S21B_AGG_TASK_WORK
#endif

static SemaphoreHandle_t s_ticks; // counting semaphore
static gptimer_handle_t s_tmr;    // handle del temporizador
static struct
{
    uint32_t isr_ticks;    // Número de ticks generados por ISR
    uint32_t takes_ok;     // Número de tomas exitosas
    uint32_t timeouts;     // Número de timeouts en tomas
    uint32_t last_wait_us; // Último tiempo de espera en us
    uint32_t max_wait_us;  // Máximo tiempo de espera en us
} s_stats;                 // estadísticas

// Simula trabajo activo durante 'us' microsegundos
static inline void busy_us(uint32_t us)
{
    int64_t t0 = esp_timer_get_time();
    while ((int64_t)esp_timer_get_time() - t0 < (int64_t)us)
    {
        __asm__ __volatile__("nop");
    }
}

// Callback de alarma del temporizador
// Incrementa el semáforo contador
...
// Tarea agregadora: consume tokens del semáforo contador
static void aggregator_task(void *arg)
{
    // Podemos configurar timeout para hacer espera indefinida o housekeeping
    TickType_t to = (TAKE_TIMEOUT_MS == 0) ? portMAX_DELAY : pdMS_TO_TICKS(TAKE_TIMEOUT_MS);
    for (;;)
    {
        int64_t t0 = esp_timer_get_time();
        if (xSemaphoreTake(s_ticks, to) == pdTRUE) // obtuvo token
        {
            int64_t t1 = esp_timer_get_time();
            uint32_t waited = (uint32_t)(t1 - t0); // tiempo de espera en us hasta obtener token
            s_stats.takes_ok++;                    // incrementa contador de tomas exitosas
            s_stats.last_wait_us = waited;         // registra último tiempo de espera
            if (waited > s_stats.max_wait_us)      // actualiza máximo tiempo de espera si aplica
                s_stats.max_wait_us = waited;
            // Simula trabajo breve por tick
            busy_us(AGG_WORK); // Simula trabajo activo
        }
        else // housekeeping por timeout (en este caso solo cuenta timeouts)
        {
            s_stats.timeouts++;
            vTaskDelay(pdMS_TO_TICKS(2)); // cede procesador en caso de timeout (backoff)
        }
    }
}

// Tarea de diagnóstico: informa estadísticas periódicamente
...
// Inicializa y ejecuta el escenario de cuenta de eventos
void escenario_timer_count_run(void)
{
    // Inicializa semáforo contador con cero eventos
    ...
    // Tareas
    BaseType_t r;
    r = xTaskCreate(aggregator_task, "AGG", 4096, NULL, AGG_PRIO, NULL);
    configASSERT(r == pdPASS);
    r = xTaskCreate(diag_task, "DIAG", 3072, NULL, 3, NULL);
    configASSERT(r == pdPASS);
